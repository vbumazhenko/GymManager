    ...
    ACTIVE {
        ...
        private boolean subscribeUser(BotContext context) {
            ...
            if (subscriptionOptional.isEmpty()) {
                ...
                if (subscriptions.size() >= context.getWorkoutType().getMaxUsers()) {

                    // Превышение количества человек. Их ставим в очередь
                    SubscribeReserve subscribeReserve = new SubscribeReserve();
                    subscribeReserve.setDate(new java.sql.Date(context.getDate().getTime()));
                    subscribeReserve.setTime(new java.sql.Time(context.getTime().getTime()));
                    subscribeReserve.setGym(context.getGym());
                    subscribeReserve.setUser(context.getCurrentUser());
                    subscribeReserve.setNotSure(context.isNotSure());
                    if (!context.isNotSure()) {
                        subscribeReserve.setCount(1);
                    }
                    subscribeReserve.setNumber(context.subscribeReserveService.getLastOrder(context) + 1);
                    context.subscribeReserveRepo.save(subscribeReserve);
                    result = true;

                    if (Utils.dateToString(context.getDate()).equals(Utils.dateToString(new Date()))) {
                        String text = "РЕЗЕРВ " + "[" + context.getCurrentUser().getName() + "](" +
